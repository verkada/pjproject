 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_verkada_vid_init(pjmedia_vid_codec_mgr *mgr,
                               pj_pool_factory *pf)
{
    pj_pool_t *pool;
    AVCodec *c;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_verkada_vid_deinit(void)
{
...
static int create_socket()
{
    ...
    if (sockfd == -1)
    {
        printf("message=\"socket creation failed... err: %s\"", strerror(errno));
        return NULL;
    }
    ...
    // read port address from /tmp/stream_port file, convert string to int
    FILE *fp = fopen("/tmp/stream_port", "r");
    if (fp == NULL)
    {
        printf("message=\"failed to open /tmp/stream_port file, err: %s\"", strerror(errno));
        char port[10];
        fgets(port, 10, fp);
        fclose(fp);
    ...
    {
        printf("message=\"connection with the server failed, port: %d, err: %s\"", servaddr.sin_port, strerror(errno));
        return NULL;
    }
    return sockfd;
}

static struct nal_parser *parser;
                                     ...
                                     pj_pool_t *pool)
{
    ...
    parser = nal_parser_create(sockfd);
    return PJ_SUCCESS;
}
...
size_t read_from_socket(ffmpeg_private *ff)
{
    int fFrameSize = 0;
    uint8_t *buffer_ptr = ff->enc_buf;
    int done = 0;

    fFrameSize = nal_parser_next_nalu(parser, ff->enc_buf, ff->enc_buf_size, &done);
    fFrameSize += 4;

    int type = buffer_ptr[4] & NAL_TYPE_MASK;
                                             ...
                                             pj_bool_t *has_more)
{
    ...
    *has_more = PJ_FALSE;
    ...
    ff->enc_frame_len = read_from_socket(ff);
    unsigned char *tempBuffer = (unsigned *)ff->enc_buf;
    int nalType = (tempBuffer[4]) & NAL_TYPE_MASK;

    ff->enc_buf_is_keyframe = nalType == NAL_TYPE_IDR_CODED_SLICE;
