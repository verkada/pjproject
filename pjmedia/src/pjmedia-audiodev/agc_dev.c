...
#define THIS_FILE                       "agc_dev.c"
#define AGC_DEVICE_NAME                "plughw:%d,%d"
 ...
 * Factory prototypes
 */
static pj_status_t agc_factory_init(pjmedia_aud_dev_factory *f);
static pj_status_t agc_factory_destroy(pjmedia_aud_dev_factory *f);
static pj_status_t agc_factory_refresh(pjmedia_aud_dev_factory *f);
static unsigned    agc_factory_get_dev_count(pjmedia_aud_dev_factory *f);
static pj_status_t agc_factory_get_dev_info(pjmedia_aud_dev_factory *f,
                                             unsigned index,
                                             pjmedia_aud_dev_info *info);
static pj_status_t agc_factory_default_param(pjmedia_aud_dev_factory *f,
                                              unsigned index,
                                              pjmedia_aud_param *param);
static pj_status_t agc_factory_create_stream(pjmedia_aud_dev_factory *f,
 ...
 * Stream prototypes
 */
static pj_status_t agc_stream_get_param(pjmedia_aud_stream *strm,
                                         pjmedia_aud_param *param);
static pj_status_t agc_stream_get_cap(pjmedia_aud_stream *strm,
                                       pjmedia_aud_dev_cap cap,
                                       void *value);
static pj_status_t agc_stream_set_cap(pjmedia_aud_stream *strm,
                                       pjmedia_aud_dev_cap cap,
                                       const void *value);
static pj_status_t agc_stream_start(pjmedia_aud_stream *strm);
static pj_status_t agc_stream_stop(pjmedia_aud_stream *strm);
static pj_status_t agc_stream_destroy(pjmedia_aud_stream *strm);
...
static pjmedia_aud_dev_factory_op agc_factory_op =
{
    &agc_factory_init,
    &agc_factory_destroy,
    &agc_factory_get_dev_count,
    &agc_factory_get_dev_info,
    &agc_factory_default_param,
    &agc_factory_create_stream,
    &agc_factory_refresh
};

static pjmedia_aud_stream_op agc_stream_op =
{
    &agc_stream_get_param,
    &agc_stream_get_cap,
    &agc_stream_set_cap,
    &agc_stream_start,
    &agc_stream_stop,
...
#if ENABLE_TRACING==0
static void null_agc_error_handler (const char *file,
                                int line,
                                const char *function,
                                int err,
                                const char *fmt,
...
static void agc_error_handler (const char *file,
                                int line,
                                const char *function,
                                int err,
                                const char *fmt,
...
static pj_status_t add_dev (struct agc_factory *af, const char *dev_name)
{
...
static void get_mixer_name(struct agc_factory *af)
{
    snd_mixer_t *handle;
...
pjmedia_aud_dev_factory* pjmedia_agc_factory(pj_pool_factory *pf)
{
    ...
    pool = pj_pool_create(pf, "agc_aud_base", 256, 256, NULL);
    af = PJ_POOL_ZALLOC_T(pool, struct agc_factory);
    af->pf = pf;
    af->base_pool = pool;
...
static pj_status_t agc_factory_init(pjmedia_aud_dev_factory *f)
{
    pj_status_t status = agc_factory_refresh(f);
...
static pj_status_t agc_factory_destroy(pjmedia_aud_dev_factory *f)
{
    struct agc_factory *af = (struct agc_factory*)f;
...
static pj_status_t agc_factory_refresh(pjmedia_aud_dev_factory *f)
{
    ...
    af->pool = pj_pool_create(af->pf, "agc_aud", 256, 256, NULL);
    af->dev_cnt = 0;
...
#if ENABLE_TRACING
    snd_lib_error_set_handler(agc_error_handler);
#else
    /* Set a null error handler prior to enumeration to suppress errors */
    snd_lib_error_set_handler(null_agc_error_handler);
#endif
...
{
    struct agc_stream* stream = (struct agc_stream*) arg;
...
static pj_status_t open_playback (struct agc_stream* stream,
                                  const pjmedia_aud_param *param)
{
    ...
    if (param->flags & PJMEDIA_AUD_DEV_CAP_OUTPUT_VOLUME_SETTING) {
        agc_stream_set_cap(&stream->base,
                            PJMEDIA_AUD_DEV_CAP_OUTPUT_VOLUME_SETTING,
                            &param->output_vol);
    }
...
static pj_status_t open_capture (struct agc_stream* stream,
                                 const pjmedia_aud_param *param)
{
    snd_pcm_hw_params_t* params;
    snd_pcm_format_t format;
                                              ...
                                              pjmedia_aud_stream **p_strm)
{
    ...
    stream = PJ_POOL_ZALLOC_T (pool, struct agc_stream);
    stream->base.op = &agc_stream_op;
    stream->pool      = pool;
    stream->af        = af;
...
static pj_status_t agc_stream_get_param(pjmedia_aud_stream *s,
                                         pjmedia_aud_param *pi)
{
    struct agc_stream *stream = (struct agc_stream*)s;
...
/* API: get capability */
static pj_status_t agc_stream_get_cap(pjmedia_aud_stream *s,
                                       pjmedia_aud_dev_cap cap,
                                       void *pval)
{
    struct agc_stream *stream = (struct agc_stream*)s;
...
static pj_status_t agc_stream_set_cap(pjmedia_aud_stream *strm,
                                       ...
                                       const void *value)
{
    struct agc_factory *af = ((struct agc_stream*)strm)->af;
...
/* API: start stream */
static pj_status_t agc_stream_start (pjmedia_aud_stream *s)
{
    struct agc_stream *stream = (struct agc_stream*)s;
...
/* API: stop stream */
static pj_status_t agc_stream_stop (pjmedia_aud_stream *s)
{
    struct agc_stream *stream = (struct agc_stream*)s;
    ...
    if (stream->pb_thread) {
        TRACE_((THIS_FILE,
                   "agc_stream_stop(%u): Waiting for playback to stop.",
                   (unsigned)syscall(SYS_gettid)));
        pj_thread_join (stream->pb_thread);
        TRACE_((THIS_FILE,
                   "agc_stream_stop(%u): playback stopped.",
                   (unsigned)syscall(SYS_gettid)));
        pj_thread_destroy(stream->pb_thread);
        stream->pb_thread = NULL;
    }
    ...
    if (stream->ca_thread) {
        TRACE_((THIS_FILE,
                   "agc_stream_stop(%u): Waiting for capture to stop.",
                   (unsigned)syscall(SYS_gettid)));
        pj_thread_join (stream->ca_thread);
        TRACE_((THIS_FILE,
                   "agc_stream_stop(%u): capture stopped.",
                   (unsigned)syscall(SYS_gettid)));
...
static pj_status_t agc_stream_destroy (pjmedia_aud_stream *s)
{
    struct agc_stream *stream = (struct agc_stream*)s;
