...
req1 = sip.RecvfromTransaction(
    "Initial request", 423, include=["REGISTER sip"], exclude=[], resp_hdr=[]
)

# Client should retry with Expires header containing special value (pjsip specific)
req2 = sip.RecvfromTransaction(
    "REGISTER retry after 423 response without Min-Expires header",
    423,
...
req3 = sip.RecvfromTransaction(
    "REGISTER retry after proper 423",
    200,
...
recvfrom_cfg = sip.RecvfromCfg("Reregistration after 423 response", pjsua, [req1, req2, req3])
