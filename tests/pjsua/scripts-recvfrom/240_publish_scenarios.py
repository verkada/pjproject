...
pjsua = (
    "--null-audio --id=sip:127.0.0.1:$PORT --registrar sip:127.0.0.1:$PORT "
...
req1 = sip.RecvfromTransaction(
    "Initial REGISTER", 200, include=["REGISTER sip"], exclude=[], resp_hdr=["Expires: 1800"]
)

# First PUBLISH, reply with 412
req2 = sip.RecvfromTransaction(
    "Initial PUBLISH, will be replied with 412", 412, include=["PUBLISH sip"], exclude=["Expires:"]
)

# Second PUBLISH
req3 = sip.RecvfromTransaction(
    "Second PUBLISH, will be replied with 200",
    200,
    include=["PUBLISH sip"],
    exclude=["Expires:"],
...
req4 = sip.RecvfromTransaction(
    "PUBLISH refresh, will be replied with 408",
    408,
...
recvfrom_cfg = sip.RecvfromCfg("PUBLISH scenarios", pjsua, [req1, req2, req3])
