...
def test_func(t):
    pjsua = t.process[0]
    # Create dialog
    dlg = sip.Dialog("127.0.0.1", pjsua.inst_param.sip_port, tcp=cfg_file.sendto_cfg.use_tcp)
    ...
    if code != cfg.resp_code:
        dlg.hangup(code)
        raise TestError("Expecting code " + str(cfg.resp_code) + " got " + str(code))
...
test = TestParam(cfg_file.sendto_cfg.name, [cfg_file.sendto_cfg.inst_param], test_func)
