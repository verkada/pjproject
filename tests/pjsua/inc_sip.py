...
req_templ = """$METHOD $TARGET_URI SIP/2.0\r
...
class Dialog:
    ...
    def update_fields(self, msg):
        if self.tcp:
            ...
            transport_param = ""
        msg = msg.replace(
            "$TARGET_URI", "sip:" + self.dst_addr + ":" + str(self.dst_port) + transport_param
        )
    ...
    def create_invite(self, sdp, extra_headers="", body=""):
        ...
        return self.create_req(
            "INVITE", sdp, branch=self.inv_branch, extra_headers=extra_headers, body=body
        )
    ...
    def wait_msg_from(self, timeout):
        ...
        while time.time() < endtime:
            ...
            if len(readset[0]) < 1 or not self.sock in readset[0]:
                if len(readset[0]) < 1:
                    print "select() timeout (will wait for " + str(
