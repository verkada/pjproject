...
class Dialog(tk.Toplevel):
    ...
    def createSipTab(self):
        ...
        ttk.Radiobutton(
            frm, text="Only offer PRACK", value=pj.PJSUA_100REL_NOT_USED, variable=self.cfgPrackUse
        ...
        ttk.Radiobutton(
            ...
            variable=self.cfgPrackUse,
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm, text="Required", value=pj.PJSUA_100REL_MANDATORY, variable=self.cfgPrackUse
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="Session Timer:").grid(row=row, column=0, sticky=tk.E, pady=2)
        ttk.Radiobutton(
            frm, text="Not offered", value=pj.PJSUA_SIP_TIMER_INACTIVE, variable=self.cfgTimerUse
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm, text="Optional", value=pj.PJSUA_SIP_TIMER_OPTIONAL, variable=self.cfgTimerUse
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm, text="Required", value=pj.PJSUA_SIP_TIMER_REQUIRED, variable=self.cfgTimerUse
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm, text="Always use", value=pj.PJSUA_SIP_TIMER_ALWAYS, variable=self.cfgTimerUse
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="Session Timer Expiration:").grid(
            row=row, column=0, sticky=tk.E, pady=2
        )
        tk.Spinbox(frm, from_=90, to=7200, textvariable=self.cfgTimerExpires, width=5).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        ...
        ttk.Checkbutton(frm, text="Enable PUBLISH", variable=self.cfgPublish).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Label(frm, text="Message Waiting Indication:").grid(
            row=row, column=0, sticky=tk.E, pady=2
        )
        ttk.Checkbutton(frm, text="Enable MWI", variable=self.cfgMwiEnabled).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Label(frm, text="NAT Traversal:").grid(row=row, column=0, sticky=tk.E, pady=2)
        ttk.Checkbutton(
            frm, text="Enable Contact Rewrite", variable=self.cfgEnableContactRewrite
        ).grid(row=row, column=1, sticky=tk.W, padx=6, pady=2)
        row += 1
        ttk.Checkbutton(frm, text="Enable Via Rewrite", variable=self.cfgEnableViaRewrite).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Checkbutton(
            frm, text="Enable SDP IP Address Rewrite", variable=self.cfgEnableSdpRewrite
        ).grid(row=row, column=1, sticky=tk.W, padx=6, pady=2)
        row += 1
        ttk.Checkbutton(
            frm, text="Enable SIP Outbound Extension", variable=self.cfgEnableSipOutbound
        ).grid(row=row, column=1, sticky=tk.W, padx=6, pady=2)
        row += 1
        ttk.Label(frm, text="UDP Keep-Alive Interval:").grid(row=row, column=0, sticky=tk.E, pady=2)
        tk.Spinbox(frm, from_=0, to=3600, textvariable=self.cfgKaInterval, width=5).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
    ...
    def createMediaTab(self):
        ...
        ttk.Radiobutton(
            frm, text="Disable", value=pj.PJMEDIA_SRTP_DISABLED, variable=self.cfgMedSrtp
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm, text="Mandatory", value=pj.PJMEDIA_SRTP_MANDATORY, variable=self.cfgMedSrtp
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm,
            ...
            variable=self.cfgMedSrtp,
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="SRTP signaling:").grid(row=row, column=0, sticky=tk.E, pady=2)
        ttk.Radiobutton(
            frm, text="Does not require secure signaling", value=0, variable=self.cfgMedSrtpSecure
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm, text="Require secure next hop (TLS)", value=1, variable=self.cfgMedSrtpSecure
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm, text="Require secure end-to-end (SIPS)", value=2, variable=self.cfgMedSrtpSecure
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="RTP transport start port:").grid(
            row=row, column=0, sticky=tk.E, pady=2
        )
        tk.Spinbox(frm, from_=0, to=65535, textvariable=self.cfgMedPort, width=5).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        ...
        tk.Spinbox(frm, from_=0, to=65535, textvariable=self.cfgMedPortRange, width=5).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        ...
        ttk.Checkbutton(frm, text="Enable", variable=self.cfgMedLockCodec).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Label(frm, text="Use IPv6:").grid(row=row, column=0, sticky=tk.E, pady=2)
        ttk.Checkbutton(frm, text="Yes", variable=self.cfgMedIpv6).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
    ...
    def createMediaNatTab(self):
        ...
        ttk.Radiobutton(
            frm, text="Default", value=pj.PJSUA_STUN_USE_DEFAULT, variable=self.cfgSipUseStun
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm, text="Disable", value=pj.PJSUA_STUN_USE_DISABLED, variable=self.cfgSipUseStun
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="Media STUN Usage:").grid(row=row, column=0, sticky=tk.E, pady=2)
        ttk.Radiobutton(
            frm, text="Default", value=pj.PJSUA_STUN_USE_DEFAULT, variable=self.cfgMediaUseStun
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm, text="Disable", value=pj.PJSUA_STUN_USE_DISABLED, variable=self.cfgMediaUseStun
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="ICE:").grid(row=row, column=0, sticky=tk.E, pady=2)
        ttk.Checkbutton(frm, text="Enable", variable=self.cfgIceEnabled).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Checkbutton(frm, text="Use aggresive nomination", variable=self.cfgIceAggressive).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Checkbutton(
            frm, text="Always re-INVITE after negotiation", variable=self.cfgAlwaysUpdate
        ).grid(row=row, column=1, sticky=tk.W, padx=6, pady=2)
        row += 1
        ttk.Checkbutton(frm, text="Disable host candidates", variable=self.cfgIceNoHostCands).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Label(frm, text="TURN:").grid(row=row, column=0, sticky=tk.E, pady=2)
        ttk.Checkbutton(frm, text="Enable", variable=self.cfgTurnEnabled).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Label(frm, text="TURN server:").grid(row=row, column=0, sticky=tk.E, pady=2)
        ttk.Entry(frm, textvariable=self.cfgTurnServer, width=20).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        ...
        ttk.Radiobutton(
            frm, text="UDP", value=pj.PJ_TURN_TP_UDP, variable=self.cfgTurnConnType
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Radiobutton(
            frm, text="TCP", value=pj.PJ_TURN_TP_TCP, variable=self.cfgTurnConnType
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="TURN username:").grid(row=row, column=0, sticky=tk.E, pady=2)
        ttk.Entry(frm, textvariable=self.cfgTurnUser, width=16).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        row += 1
        ttk.Label(frm, text="TURN password:").grid(row=row, column=0, sticky=tk.E, pady=2)
        ttk.Entry(frm, textvariable=self.cfgTurnPasswd, show="*", width=16).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
    ...
    def onOk(self):
        ...
        self.cfg.mediaConfig.ipv6Use = (
            pj.PJSUA_IPV6_ENABLED if self.cfgMedIpv6.get() else pj.PJSUA_IPV6_DISABLED
        )
