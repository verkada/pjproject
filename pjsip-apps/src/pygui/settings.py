...
class Dialog(tk.Toplevel):
    ...
    def createBasicTab(self):
        ...
        self.cfgLogAppend = tk.BooleanVar(
            value=True if (self.cfg.epConfig.logConfig.fileFlags & pj.PJ_O_APPEND) else False
        )
        ...
        ttk.Label(frm, text=self.cfg.epConfig.uaConfig.userAgent).grid(
            row=row, column=1, sticky=tk.W, pady=2, padx=6
        )
        row += 1
        ttk.Label(frm, text="Max calls:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        ttk.Label(frm, text=str(self.cfg.epConfig.uaConfig.maxCalls)).grid(
            row=row, column=1, sticky=tk.W, pady=2, padx=6
        )
        row += 1
        ttk.Label(frm, text="Log file:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        ttk.Entry(frm, textvariable=self.cfgLogFile, width=32).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        row += 1
        ttk.Checkbutton(frm, text="Append log file", variable=self.cfgLogAppend).grid(
    ...
    def createNetworkTab(self):
        ...
        ttk.Checkbutton(frm, text="Enable UDP transport", variable=self.cfgUdpEnabled).grid(
            row=row, column=0, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Label(frm, text="UDP port:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        tk.Spinbox(frm, from_=0, to=65535, textvariable=self.cfgUdpPort, width=5).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        ...
        ttk.Checkbutton(frm, text="Enable TCP transport", variable=self.cfgTcpEnabled).grid(
            row=row, column=0, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Label(frm, text="TCP port:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        tk.Spinbox(frm, from_=0, to=65535, textvariable=self.cfgTcpPort, width=5).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        ttk.Label(frm, text="(0 for any)").grid(row=row, column=1, sticky=tk.E, pady=6, padx=6)
        row += 1
        # ttk.Label(frm, text='TLS transport:').grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        ttk.Checkbutton(frm, text="Enable TLS transport", variable=self.cfgTlsEnabled).grid(
            row=row, column=0, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Label(frm, text="TLS port:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        tk.Spinbox(frm, from_=0, to=65535, textvariable=self.cfgTlsPort, width=5).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        ttk.Label(frm, text="(0 for any)").grid(row=row, column=1, sticky=tk.E, pady=6, padx=6)
        row += 1
        ttk.Label(frm, text="TLS CA file:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        ttk.Entry(frm, textvariable=self.cfgTlsCaFile, width=32).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        row += 1
        ttk.Label(frm, text="TLS cert file:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        ttk.Entry(frm, textvariable=self.cfgTlsCertFile, width=32).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        row += 1
        ttk.Checkbutton(frm, text="TLS verify server", variable=self.cfgTlsVerifyServer).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Checkbutton(frm, text="TLS verify client", variable=self.cfgTlsVerifyClient).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ...
        ttk.Entry(frm, textvariable=self.cfgNameserver, width=32).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        row += 1
        ttk.Label(frm, text="STUN Server:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        ttk.Entry(frm, textvariable=self.cfgStunServer, width=32).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        row += 1
        ttk.Checkbutton(
            frm, text="Ignore STUN failure at startup", variable=self.cfgStunIgnoreError
    ...
    def createMediaTab(self):
        ...
        ttk.Label(frm, text=str(self.cfg.epConfig.medConfig.maxMediaPorts)).grid(
            row=row, column=1, sticky=tk.W, pady=2, padx=6
        )
        row += 1
        ttk.Label(frm, text="Core clock rate:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        tk.Spinbox(
            frm, from_=8000, to=48000, increment=8000, textvariable=self.cfgClockrate, width=5
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="Snd device clock rate:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        tk.Spinbox(
            frm, from_=0, to=48000, increment=8000, textvariable=self.cfgSndClockrate, width=5
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        ...
        tk.Spinbox(
            frm, from_=10, to=400, increment=10, textvariable=self.cfgAudioPtime, width=3
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="RTP ptime:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        tk.Spinbox(
            frm, from_=20, to=400, increment=10, textvariable=self.cfgCodecPtime, width=3
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="Media quality (1-10):").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        tk.Spinbox(frm, from_=1, to=10, textvariable=self.cfgMediaQuality, width=5).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        row += 1
        ttk.Label(frm, text="VAD:").grid(row=row, column=0, sticky=tk.E, pady=2, padx=8)
        ttk.Checkbutton(frm, text="Enable", variable=self.cfgVad).grid(
            row=row, column=1, sticky=tk.W, padx=6, pady=2
        )
        row += 1
        ttk.Label(frm, text="Echo canceller tail length:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        tk.Spinbox(
            frm, from_=0, to=400, increment=10, textvariable=self.cfgEcTailLen, width=3
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        ttk.Label(frm, text="(ms, 0 to disable)").grid(
            row=row, column=1, sticky=tk.E, pady=6, padx=6
        )
