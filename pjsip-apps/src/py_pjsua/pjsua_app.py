...
def on_reg_state(acc_id):
    ...
    if acc_info.status != 0 and acc_info.status != 200:
        write_log(
            3, "Account " + cmd + " failed: rc=" + ` acc_info.status ` + " " + acc_info.status_text
        )
    else:
...
def on_call_transfer_request(call_id, dst, code):
    write_log(
        3, "Call transfer request from " + ` call_id ` + " to " + dst + " with code " + ` code `
    )
...
def app_init():
    ...
    status, transport_id = py_pjsua.transport_create(py_pjsua.PJSIP_TRANSPORT_UDP, transport_cfg)
    if status != 0:
...
def conf_list():
    ...
    for port in ports:
        ...
        print "Port #" + ` info.slot_id ` + "[" + ` (info.clock_rate / 1000) ` + "KHz/" + ` (
            info.samples_per_frame * 1000 / info.clock_rate
        ) ` + "ms] " + info.name + " transmitting to: " + txlist
