...
class Member:
    ...
    def join_call(self, call):
        ...
        msg = "%(uri)s is attempting to join the voice conference" % {"uri": self.uri}
        self.bot.DEBUG(msg + "\n", INFO)
    ...
    def on_call_state(self, call):
        ...
        if ci.state == pj.CallState.DISCONNECTED:
            ...
            msg = "%(uri)s has joined the voice conference" % {"uri": self.uri}
            self.bot.DEBUG(msg + "\n", INFO)
            self.bot.broadcast_pager(None, msg)
    ...
    def on_call_dtmf_digit(self, call, digits):
        msg = "%(uri)s sent DTMF digits %(dig)s" % {"uri": self.uri, "dig": digits}
        self.bot.broadcast_pager(None, msg)

    def on_call_transfer_request(self, call, dst, code):
        msg = "%(uri)s is transferring the call to %(dst)s" % {"uri": self.uri, "dst": dst}
        self.bot.broadcast_pager(None, msg)
        return 202
    ...
    def on_call_replace_request(self, call, code, reason):
        msg = "%(uri)s is requesting call replace" % {"uri": self.uri}
        self.bot.broadcast_pager(None, msg)
        return (code, reason)

    def on_call_replaced(self, call, new_call):
        msg = "%(uri)s call is replaced" % {"uri": self.uri}
        self.bot.broadcast_pager(None, msg)

    def on_pres_state(self, buddy):
        old_bi = self.bi
        self.bi = buddy.info()
        msg = "%(uri)s status is %(st)s" % {"uri": self.uri, "st": self.bi.online_text}
        self.bot.DEBUG(msg + "\n", INFO)
        ...
        if self.bi.sub_state == pj.SubscriptionState.ACTIVE:
            ...
            self.bi.sub_state == pj.SubscriptionState.NULL
            or self.bi.sub_state == pj.SubscriptionState.TERMINATED
            or self.bi.sub_state == pj.SubscriptionState.UNKNOWN
        ):
            self.buddy.delete()
            self.buddy = None
            if self.in_chat:
...
""" % {
            "uri": ai.uri,
            "pres": ai.online_text,
            "reg_st": ai.reg_status,
            "reg_res": ai.reg_reason,
        }
        return msg
    ...
    def on_reg_state(self):
        ai = self.acc.info()
        self.DEBUG(
            "Registration state: %(code)d/%(reason)s\n"
            % {"code": ai.reg_status, "reason": ai.reg_reason},
            INFO,
        )
    ...
    def on_pager(self, from_uri, contact, mime_type, body):
        ...
        if not self.handle_cmd(None, from_uri, body):
            self.acc.send_pager(
                from_uri,
                "You have not joined the chat room. Type '--join' to join or '--help' for the help",
            )
