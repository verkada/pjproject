...
compile_flags = [
    ...
    [
        "HAS_PJLIB",
        "PJLIB (pool, data structures, hash tables, ioqueue, socket, timer heap, etc.). "
    ...
    [
        "HAS_PJSIP_CORE_MSG_UTIL",
        "PJSIP Core - Stateless operations, SIP SRV, server resolution and fail-over",
    ],
...
def print_html_report():
    ...
    for i in range(1, len(exe_size)):
        ...
        if e[1] <> "":
            output.write(
                '  <TD align="right">' + ` string.atoi(e[2]) - string.atoi(prev[2]) ` + "</TD>\n"
            )
            output.write(
                '  <TD align="right">' + ` string.atoi(e[3]) - string.atoi(prev[3]) ` + "</TD>\n"
            )
            output.write(
                '  <TD align="right">' + ` string.atoi(e[4]) - string.atoi(prev[4]) ` + "</TD>\n"
            )
            ...
            output.write('  <TD align="right">&nbsp;</TD>\n')
            output.write(
                "  <TD><strong>" + e[5] + ": .text=" + e[2] + ", .data=" + e[3] + ", .bss=" + e[4]
            )
            output.write(
                "\n </strong> <BR>(Size minus empty application size: "
                + ".text="
                + ` string.atoi(e[2])
                - string.atoi(empty_size[2]) `
                + ", .data="
                + ` string.atoi(e[3])
                - string.atoi(empty_size[3]) `
                + ", .data="
                + ` string.atoi(e[4])
                - string.atoi(empty_size[4]) `
                + ")\n"
            )
            output.write(" </TD>\n")
